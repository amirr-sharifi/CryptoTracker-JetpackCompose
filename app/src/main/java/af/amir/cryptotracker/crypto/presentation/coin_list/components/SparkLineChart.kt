package af.amir.cryptotracker.crypto.presentation.coin_list.components

import androidx.compose.foundation.Canvas
import androidx.compose.foundation.layout.fillMaxWidth
import androidx.compose.foundation.layout.height
import androidx.compose.material3.MaterialTheme
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.Path
import androidx.compose.ui.graphics.drawscope.Stroke
import androidx.compose.ui.tooling.preview.Preview
import androidx.compose.ui.unit.dp

@Composable
fun SparkLineChart(
    prices: List<Double>,
    modifier: Modifier = Modifier,
    strokeSize : Float =2f,
) {
    if (prices.isEmpty()) return
    val limitPrices = if (prices.size > 30) prices.takeLast(30) else prices
    val points = limitPrices.mapIndexed { index, price ->
        index.toFloat() to price.toFloat()
    }

    val maxY = points.maxOf { it.second }
    val minY = points.minOf { it.second }
    val rangeY = maxY - minY

    val maxX = points.maxOf { it.first }

    val pathColor = MaterialTheme.colorScheme.primary
    Canvas(modifier) {

        val path = Path()

        points.forEachIndexed { index, (x, y) ->
            val px = (x / maxX) * size.width
            val py = size.height - ((y - minY) / rangeY) * size.height

            if (index == 0) {
                path.moveTo(px, py)
            } else {
                path.lineTo(px, py)
            }
        }

        drawPath(path, pathColor, style = Stroke(strokeSize))

    }
}


@Preview(showBackground = true)
@Composable
private fun SparklinePreview() {
    val p = listOf(
        91477.54598695686,
        91455.11137688499,
        91440.20549816827,
        91314.55693607975,
        91176.29202753108,
        91159.28211972128,
        90359.86873169734,
        87107.57804601092,
        87209.3202760441,
        86607.95806802201,
        86347.62398034424,
        85788.16096067103,
        85996.54791593562,
        86248.96829120435,
        86509.66964426964,
        86828.86588088673,
        86711.05550833189,
        86632.27972318193,
        86262.5310475135,
        85452.87201488385,
        85992.51478024718,
        86023.91194853347,
        84634.95418600549,
        84553.51608407828,
        84958.90011684994,
        85230.8975559526,
        85003.07161606019,
        85607.92665356252,
        86425.82291576332,
        86642.17601166596,
        86281.49519776918,
        86589.01329234689,
        86431.01161842069,
        86639.7190996139,
        86979.2416342632,
        86999.26753257588,
        87009.82948356986,
        87094.35617736231,
        87032.32838561568,
        86418.6233251056,
        86789.61201168034,
        87283.4200046968,
        87372.145785899,
        87310.28717151324,
        87676.27574943205,
        89212.56733407508,
        90929.17033076797,
        90988.35289671065,
        91516.49010026302,
        92003.14962051474,
        91925.7686665393,
        91006.60257571642,
        91642.26634875845,
        91972.84712515515,
        91320.07942505727,
        91587.10768114813,
        92305.80571530102,
        92804.99277281489,
        92694.94244803755,
        93148.20270941404,
        93673.50779118965,
        93405.41156538975,
        92997.38296608128,
        92758.94715524407,
        93187.76365353932,
        92967.95172916472,
        93014.56287493998,
        92955.9934046737,
        93155.5710477267,
        92689.18467540754,
        92462.71285159403,
        92358.91089703525,
        92991.34046933519,
        92701.01925701783,
        93042.76426891205,
        92956.21718894965,
        93748.59078386276,
        93717.47149577878,
        93428.59451478458,
        93035.40920589627,
        93194.86563225763,
        93855.83582607246,
        93567.32981233204,
        93361.27533623583,
        92899.65881208924,
        93124.7224503544,
        93344.22464629909,
        93162.29916189116,
        93576.35582972465,
        93249.89554012878,
        92962.28988355577,
        92982.88944879391,
        92532.02394137291,
        92053.41845475078,
        92596.47508142228,
        92349.10012511045,
        92448.2278390964,
        92131.16632716786,
        92068.62882915599,
        92469.36285556007,
        92194.78324592086,
        92349.58095934307,
        92140.70419795791,
        92302.08974651141,
        92370.2072229294,
        92527.8315926902,
        92302.35560866309,
        92070.48457147281,
        91921.18210371182,
        92263.3299565551,
        92244.84957275762,
        92114.99015350078,
        91133.25125696731,
        91321.13588701593,
        91367.32047901883,
        91261.71327040567,
        90495.89880179014,
        90239.62309417447,
        90691.05834616034,
        88838.81256963374,
        89320.67330009381,
        88971.13700892155,
        89661.0097557126,
        89319.18628091525,
        89168.6188930498,
        89246.76255414792,
        89307.09154512963,
        89411.87151142707,
        89313.00635292601,
        89404.64876063257,
        89659.75706048295,
        89670.35753876393,
        89596.5402589313,
        89700.0501824711,
        89610.67940053929,
        89322.01341173225,
        89590.58493068343,
        89523.29215522746,
        89626.21664015269,
        89598.52998975923,
        89686.06129936306,
        90081.81117280423,
        89791.26875713006,
        89711.11278041216,
        89688.726231859,
        89594.48934194396,
        89463.06998584204,
        89554.86879282963,
        89447.79992594938,
        89301.79183131723,
        89244.7620814118,
        89442.08598085925,
        89380.5784461144,
        89591.40163921085,
        89728.67942532501,
        89553.15476748442,
        89501.47167967241,
        89592.78832989243,
        89375.83415426753,
        89428.99362624755,
        89125.8674744549,
        89260.10758592366,
        89179.22240236348,
        89481.7211308164,
        89062.22627235795,
        88202.39414470572,
        89472.58036849847,
        89818.66878296863
    )
    SparkLineChart(p,
        Modifier
            .fillMaxWidth()
            .height(90.dp))
}


